...
func TestDASRekey(t *testing.T) {
...

	{
		...
		nodeA, err := arbnode.CreateNode(ctx, l2stackA, l2chainDb, l2arbDb, NewFetcherFromConfig(l1NodeConfigA), l2blockchain, l1client, l1info.ChainId, addresses, sequencerTxOptsPtr, nil, feedErrChan)
		Require(t, err)
...
func TestDASComplexConfigAndRestMirror(t *testing.T) {
	...
	l1info, l1client, _, l1stack := createTestL1BlockChain(t, nil)
	defer requireClose(t, l1stack)
	l1Reader := headerreader.New(l1client, l1info.ChainId, func() *headerreader.Config { return &headerreader.TestConfig })
	l1Reader.Start(ctx)
	...
	sequencerTxOpts := l1info.GetDefaultTransactOpts("Sequencer", ctx)
	sequencerTxOptsPtr := &sequencerTxOpts
	nodeA, err := arbnode.CreateNode(ctx, l2stackA, l2chainDb, l2arbDb, NewFetcherFromConfig(l1NodeConfigA), l2blockchain, l1client, l1info.ChainId, addresses, sequencerTxOptsPtr, dataSigner, feedErrChan)
	Require(t, err)
	Require(t, nodeA.Start(ctx))
	l2clientA := ClientForStack(t, l2stackA)
